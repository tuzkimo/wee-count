...
- Produces (Task 5/6 consume) — 精确签名：
  ...
  - `function computeRange(unit: PeriodUnit, offset: number): { range: ReportRange; prevRange: ReportRange; label: string; granularity: Granularity }`
    - offset 0 = 当前周期；month 标签 `2026年8月`；quarter `2026年Q3`；year `2026年`；twelveMonths `2025年9月–2026年8月`（end = 下月首日，窗口 = **含当前月**的 12 个月；标签末月 = end − 1 月）
  ...
  - `function buildNetAssetSeries(baseline: number, flows: { bucket: string; income: number; expense: number }[], initials: { bucket: string; v: number }[], labels: string[]): number[]` — 返回纯数组：baseline + 逐桶累加(income−expense) + 该桶新增账户期初；由 `getNetAssetSeries` 包成 `{ labels, values }`（Task 4 评审确认，与测试契约一致）
...
describe("computeRange", () => {
  ...
  it("twelveMonths window includes the current month and spans 12 months", () => {
    ...
    // 标签为 "YYYY年M月–YYYY年M月"，末月 = end − 1 月
