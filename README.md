...
- **应用锁**：6 位数字 PIN 或九宫格图案解锁，冷启动必须解锁，切到后台超过设定时长自动重新锁定，可选 Android 截屏防护（FLAG_SECURE，默认开启）；它是界面级门禁，不改变本地数据文件本身（数据文件仍是明文），能读取设备存储的人可以绕过；忘记 PIN 时的出路是在解锁页用账户密码就地设置一把新锁，因此任何本机账户的凭据都可以重置这把设备级应用锁，且该出路没有独立的失败限流（5 次失败计数只作用于 PIN/图案，且仅存内存，杀进程即清零）
