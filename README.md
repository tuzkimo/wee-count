...
- **筛选**：账户、日期范围、分类、标签、成员任意组合过滤，条件写入 URL 并回填首页；日期范围是日历连选（先点起点再点终点），内置本周/本月/上月/今年/上年等快捷范围
- **团队共享账本**：创建/加入团队（邀请码），成员管理与成员备注别名
- **备份与恢复**：一键导出全量数据为密码加密备份文件（PBKDF2-SHA256 + AES-256-GCM），重装/换机后从欢迎页或「我的」恢复为全新账户，不影响现有数据
- **多用户与同步**：本地多账户独立数据库；自托管后端后多设备同步（LWW 合并 + 服务端单调游标增量），服务下线自动降级重连
- **隐私**：本地模式数据不出设备；自托管时数据只在你自己的服务器；可选 Android 截屏防护（FLAG_SECURE，默认开启，开启后截图全黑、最近任务列表不显示内容）
- **应用锁**：6 位数字 PIN 或九宫格图案解锁，冷启动必须解锁，切到后台超过设定时长自动重新锁定；首次登录/注册后会询问一次是否开启（设备级只问一次，拒绝后不再打扰）；它与截屏防护是两件独立的事，后者不开应用锁也能单独开关；它是界面级门禁，不改变本地数据文件本身（数据文件仍是明文），能读取设备存储的人可以绕过；忘记 PIN 时的出路是在解锁页用账户密码就地设置一把新锁，因此任何本机账户的凭据都可以重置这把设备级应用锁，且该出路没有独立的失败限流（5 次失败计数只作用于 PIN/图案，且仅存内存，杀进程即清零）
